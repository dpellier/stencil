...
export function setAccessor(plt: PlatformApi, elm: any, name: string, oldValue: any, newValue: any, isSvg: boolean, i?: number, ilen?: number) {
  if (name === 'class' && !isSvg) {
    // Class
    if (oldValue !== newValue) {
      ...
      for (i = 0, ilen = newList.length; i < ilen; i += 1) {
