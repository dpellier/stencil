import { BuildCtx, CompilerCtx, Config, HydrateResults, PrerenderConfig, PrerenderLocation } from '../../declarations';
...
export async function prerenderApp(config: Config, compilerCtx: CompilerCtx, buildCtx: BuildCtx) {
  if (!config.prerender) {
  ...
  return runPrerenderApp(config, compilerCtx, buildCtx, prerenderQueue, indexHtml);
}


async function runPrerenderApp(config: Config, compilerCtx: CompilerCtx, buildCtx: BuildCtx, prerenderQueue: PrerenderLocation[], indexHtml: string) {
  ...
  try {
    ...
    await generateHostConfig(config, compilerCtx, buildCtx.entryModules, hydrateResults);
